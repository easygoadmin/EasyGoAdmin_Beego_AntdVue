...
func (ctl *ItemController) Detail() {
	// 记录ID
...
func (ctl *ItemController) GetItemList() {
	// 查询站点列表
	list := make([]models.Item, 0)
	orm.NewOrm().QueryTable(new(models.Item)).Filter("status", 1).Filter("mark", 1).OrderBy("sort").All(&list)
	// 返回结果
	ctl.JSON(common.JsonResult{
		Code: 0,
		Msg:  "查询成功",
