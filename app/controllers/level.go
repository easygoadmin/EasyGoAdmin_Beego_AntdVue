...
func (ctl *LevelController) Detail() {
	// 记录ID
...
func (ctl *LevelController) GetLevelList() {
	list := make([]models.Level, 0)
	orm.NewOrm().QueryTable(new(models.Level)).Filter("status", 1).Filter("mark", 1).OrderBy("sort").All(&list)
	// 返回结果
	ctl.JSON(common.JsonResult{
		Code: 0,
		Msg:  "查询成功",
