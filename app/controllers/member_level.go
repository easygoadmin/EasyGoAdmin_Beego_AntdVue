...
func (ctl *MemberLevelController) Detail() {
	// 记录ID
...
func (ctl *MemberLevelController) GetMemberLevelList() {
	list := make([]models.MemberLevel, 0)
	orm.NewOrm().QueryTable(new(models.MemberLevel)).Filter("mark", 1).OrderBy("sort").All(&list)
	// 返回结果
	ctl.JSON(common.JsonResult{
		Code: 0,
		Msg:  "查询成功",
