...
// 获取系统菜单
func (ctl *IndexController) Menu() {
	// 获取菜单列表
	menuList := services.Menu.GetPermissionMenuList(utils.Uid(ctl.Ctx))
	// 返回结果
...
// 获取用户信息
func (ctl *IndexController) User() {
	// 获取用户信息
	user := &models.User{Id: utils.Uid(ctl.Ctx)}
	err := user.Get()
	if err != nil {
		ctl.JSON(common.JsonResult{
			Code: -1,
	...
	// 获取权限节点
	permissionList := services.Menu.GetPermissionsList(utils.Uid(ctl.Ctx))
...
func (ctl *IndexController) UpdateUserInfo() {
	// 参数验证
