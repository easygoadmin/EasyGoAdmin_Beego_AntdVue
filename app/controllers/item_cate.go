...
func (ctl *ItemCateController) Detail() {
	// 记录ID
...
func (ctl *ItemCateController) GetCateList() {
	list := make([]models.ItemCate, 0)
	orm.NewOrm().QueryTable(new(models.ItemCate)).Filter("status", 1).Filter("mark", 1).OrderBy("sort").All(&list)
	// 返回结果
	ctl.JSON(common.JsonResult{
		Code: 0,
		Msg:  "查询成功",
