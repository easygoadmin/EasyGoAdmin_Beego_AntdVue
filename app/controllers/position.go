...
func (ctl *PositionController) GetPositionList() {
	// 查询岗位列表
	list := make([]models.Position, 0)
	orm.NewOrm().QueryTable(new(models.Position)).Filter("status", 1).Filter("mark", 1).OrderBy("sort").All(&list)
	// 返回结果
	ctl.JSON(common.JsonResult{
		Code: 0,
		Msg:  "查询成功",
